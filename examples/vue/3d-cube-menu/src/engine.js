...
export const FaceSelected = engine.event('FaceSelected');
export const SnapToFace = engine.event('SnapToFace');
export const SnapDone = engine.event('SnapDone');
/* ------------------------------------------------------------------ */
/*  Rotation signals + springs                                        */
/* ------------------------------------------------------------------ */
export const rotationXTarget = engine.signal(DragMove, 0, (prev, { dy }) => prev + dy * 0.4);
export const rotationYTarget = engine.signal(DragMove, 0, (prev, { dx }) => prev + dx * 0.4);
engine.signalUpdate(rotationXTarget, SnapToFace, (_prev, face) => {
    const snaps = { 4: -90, 5: 90 };
    return snaps[face] ?? 0;
});
engine.signalUpdate(rotationYTarget, SnapToFace, (_prev, face) => {
    const snaps = { 0: 0, 1: -90, 2: -180, 3: -270 };
    return snaps[face] ?? 0;
});
export const rotXSpring = engine.spring(rotationXTarget, { stiffness: 120, damping: 20 });
export const rotYSpring = engine.spring(rotationYTarget, { stiffness: 120, damping: 20 });
export const selectedFace = engine.signal(FaceSelected, -1, (_prev, idx) => idx);
/* ------------------------------------------------------------------ */
/*  Snap logic                                                        */
/* ------------------------------------------------------------------ */
engine.on(DragEnd, () => {
