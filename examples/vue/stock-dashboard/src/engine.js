...
export const DismissAlert = engine.event('DismissAlert');
/* ------------------------------------------------------------------ */
/*  Initial stocks                                                    */
/* ------------------------------------------------------------------ */
...
/*  Signals                                                           */
/* ------------------------------------------------------------------ */
export const stocks = engine.signal(PriceUpdate, INITIAL_STOCKS, (prev, { symbol, price }) => prev.map(s => {
    if (s.symbol !== symbol)
        return s;
...
let alertId = 0;
export const alerts = engine.signal(AlertTriggered, [], (prev, alert) => [alert, ...prev].slice(0, 10));
