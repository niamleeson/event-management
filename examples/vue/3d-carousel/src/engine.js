...
export const ItemSelected = engine.event('ItemSelected');
...
/* ------------------------------------------------------------------ */
export const rotationTarget = engine.signal(DragMove, 0, (prev, { dx }) => prev + dx * 0.3);
// Snap on item select
engine.signalUpdate(rotationTarget, ItemSelected, (_prev, idx) => -idx * ANGLE_STEP);
// Spring-driven smooth rotation
export const rotationSpring = engine.spring(rotationTarget, { stiffness: 80, damping: 18 });
/* ------------------------------------------------------------------ */
/*  Selected item                                                     */
/* ------------------------------------------------------------------ */
export const selectedItem = engine.signal(ItemSelected, 0, (_prev, idx) => idx);
/* ------------------------------------------------------------------ */
/*  Selected item springs forward (translateZ boost)                  */
/* ------------------------------------------------------------------ */
export const selectedZTargets = [];
export const selectedZSprings = [];
for (let i = 0; i < ITEM_COUNT; i++) {
    const zt = engine.signal(ItemSelected, 0, (_prev, idx) => idx === i ? 60 : 0);
