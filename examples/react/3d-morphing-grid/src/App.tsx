...
import type { EventType, Signal, TweenValue } from '@pulse/core'
...
function getShapeTargets(shape: Shape, row: number, col: number): { rotX: number; rotY: number; tz: number; scale: number; radius: number } {
  const cx = (GRID - 1) / 2
  const cy = (GRID - 1) / 2
  const dx = col - cx
  const dy = row - cy
...
function cellDistFromCenter(index: number): number {
...
const cellMorphDones: EventType<number>[] = []

for (let i = 0; i < CELL_COUNT; i++) {
  cellMorphStarts.push(engine.event(`CellMorphStart_${i}`))
  cellMorphDones.push(engine.event<number>(`CellMorphDone_${i}`))
}
...
const cellTargets: Array<{ rotX: number; rotY: number; tz: number; scale: number; radius: number }> = []
const cellFroms: Array<{ rotX: number; rotY: number; tz: number; scale: number; radius: number }> = []

// Tween arrays: each cell has 5 tweens
const rotXTweens: TweenValue[] = []
const rotYTweens: TweenValue[] = []
const tzTweens: TweenValue[] = []
const scaleTweens: TweenValue[] = []
const radiusTweens: TweenValue[] = []
...
const currentShape: Signal<Shape> = engine.signal(MorphTrigger, 'flat' as Shape, (_prev, shape) => shape)

/* ------------------------------------------------------------------ */
/*  Auto-cycle mode                                                   */
/* ------------------------------------------------------------------ */

const autoCycling: Signal<boolean> = engine.signal(AutoCycleToggle, false, (prev) => !prev)
...
export default function App() {
  ...
  const triggerShape = useCallback((s: Shape) => {
    emit(MorphTrigger, s)
  }, [emit])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 40 }}>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'center' }}>
        ...
        <button
          onClick={() => emit(AutoCycleToggle, undefined)}
          style={{
      ...
      <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 13 }}>
        Current: <span style={{ color: 'rgba(255,255,255,0.7)' }}>{shape}</span>
      </p>
    </div>
  )
}
