...
function Cell({ row, col }: { row: number; col: number }) {
  ...
  return (
    <td
      style={styles.cell(isSelected, hasError)}
      onClick={handleClick}
    ...
    >
      {cell.computed}
      {hasError && isSelected && (
        <div style={styles.errorTooltip}>{cell.error}</div>
      )}
    </td>
  )
}
        ...
        <thead>
          <tr>
            ...
            {Array.from({ length: COLS }, (_, c) => (
              <th key={c} style={styles.colHeader}>{colLabel(c)}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {Array.from({ length: ROWS }, (_, r) => (
            <tr key={r}>
              <td style={styles.rowHeader}>{r + 1}</td>
              {Array.from({ length: COLS }, (_, c) => (
...
function StatusBar() {
  ...
  return (
    <div style={styles.statusBar}>
      <span>
        {filledCount} cells filled | {formulaCount} formulas | {errorCount} errors
      </span>
      <span>
