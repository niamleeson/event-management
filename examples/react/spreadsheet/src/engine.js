...
function evaluateFormula(formula, grid, selfRow, selfCol) {
    ...
    if (sumMatch) {
        ...
        if (!rangeCoords)
            throw new Error('Invalid range in SUM');
    ...
    if (avgMatch) {
        ...
        if (!rangeCoords || rangeCoords.length === 0)
            throw new Error('Invalid range in AVG');
    ...
    if (minMatch) {
        ...
        if (!rangeCoords || rangeCoords.length === 0)
            throw new Error('Invalid range in MIN');
    ...
    if (maxMatch) {
        ...
        if (!rangeCoords || rangeCoords.length === 0)
            throw new Error('Invalid range in MAX');
    ...
    let resolved = expr.replace(/[A-H][1-8]/gi, (match) => {
        ...
        if (!ref)
            return '0';
        if (ref.row === selfRow && ref.col === selfCol)
...
// Event declarations
// ---------------------------------------------------------------------------
export const CellEdited = engine.event('CellEdited');
export const CellComputed = engine.event('CellComputed');
export const CellSelected = engine.event('CellSelected');
export const FormulaError = engine.event('FormulaError');
// ---------------------------------------------------------------------------
// Pipe: CellEdited -> CellComputed (with formula evaluation)
// ---------------------------------------------------------------------------
engine.pipe(CellEdited, [CellComputed, FormulaError], (payload) => {
...
// Signals
// ---------------------------------------------------------------------------
export const cells = engine.signal(CellEdited, makeEmptyGrid(), (prev, payload) => {
...
export const selectedCell = engine.signal(CellSelected, { row: 0, col: 0 }, (_prev, coord) => coord);
