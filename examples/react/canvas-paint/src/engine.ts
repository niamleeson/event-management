...
engine.on(ToolChanged, (t) => { currentTool = t; engine.emit(CurrentToolChanged, t) })
engine.on(ColorChanged, (c) => { currentColor = c; engine.emit(CurrentColorChanged, c) })
engine.on(SizeChanged, (s) => { brushSize = s; engine.emit(BrushSizeChanged, s) })
engine.on(LayerSelected, (id) => { activeLayer = id; engine.emit(ActiveLayerChanged, id) })

engine.on(StrokeStart, ({ x, y }) => { lastMoveTime = Date.now(); currentStroke = { id: `s-${++strokeCounter}`, tool: currentTool, color: currentTool === 'eraser' ? '#1a1a2e' : currentColor, size: brushSize, points: [{ x, y, pressure: 1 }], layerId: activeLayer } })
engine.on(StrokeMove, ({ x, y, pressure }) => { if (!currentStroke) return; const now = Date.now(), dt = now - lastMoveTime; lastMoveTime = now; const lp = currentStroke.points[currentStroke.points.length - 1]; const d = Math.sqrt((x - lp.x) ** 2 + (y - lp.y) ** 2); const sp = dt > 0 ? d / dt : 0; currentStroke = { ...currentStroke, points: [...currentStroke.points, { x, y, pressure: pressure ?? Math.max(0.2, Math.min(1, 1 - sp * 0.3)) }] } })
engine.on(StrokeEnd, () => { if (!currentStroke) return; if (currentStroke.points.length > 1) { strokes = [...strokes, currentStroke]; undoArr = [...undoArr, currentStroke]; redoArr = []; engine.emit(StrokesChanged, [...strokes]); engine.emit(UndoStackChanged, [...undoArr]); engine.emit(RedoStackChanged, []) }; currentStroke = null })
engine.on(UndoStroke, () => { if (!undoArr.length) return; const l = undoArr[undoArr.length - 1]; undoArr = undoArr.slice(0, -1); redoArr = [...redoArr, l]; strokes = strokes.filter(s => s.id !== l.id); engine.emit(StrokesChanged, [...strokes]); engine.emit(UndoStackChanged, [...undoArr]); engine.emit(RedoStackChanged, [...redoArr]) })
engine.on(RedoStroke, () => { if (!redoArr.length) return; const l = redoArr[redoArr.length - 1]; redoArr = redoArr.slice(0, -1); undoArr = [...undoArr, l]; strokes = [...strokes, l]; engine.emit(StrokesChanged, [...strokes]); engine.emit(UndoStackChanged, [...undoArr]); engine.emit(RedoStackChanged, [...redoArr]) })
engine.on(ClearCanvas, () => { strokes = []; undoArr = []; redoArr = []; currentStroke = null; engine.emit(StrokesChanged, []); engine.emit(UndoStackChanged, []); engine.emit(RedoStackChanged, []) })
engine.on(LayerAdded, () => { layerCounter++; layers = [...layers, { id: layerCounter, name: `Layer ${layerCounter}`, visible: true }]; engine.emit(LayersChanged, [...layers]); engine.emit(LayerSelected, layerCounter) })
engine.on(LayerToggled, (id) => { layers = layers.map(l => l.id === id ? { ...l, visible: !l.visible } : l); engine.emit(LayersChanged, [...layers]) })
