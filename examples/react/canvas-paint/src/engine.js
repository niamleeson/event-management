...
export const ClearCanvas = engine.event('ClearCanvas');
...
// Internal
export const RenderRequest = engine.event('RenderRequest');
// ---------------------------------------------------------------------------
// Signals
// ---------------------------------------------------------------------------
export const currentTool = engine.signal(ToolChanged, 'brush', (_prev, tool) => tool);
export const currentColor = engine.signal(ColorChanged, '#3b82f6', (_prev, color) => color);
export const brushSize = engine.signal(SizeChanged, 4, (_prev, size) => size);
export const strokes = createSignal([]);
engine['_signals'].push(strokes);
export const undoStack = createSignal([]);
engine['_signals'].push(undoStack);
export const redoStack = createSignal([]);
engine['_signals'].push(redoStack);
export const layers = createSignal([
...
export const currentStroke = createSignal(null);
...
engine.on(LayerAdded, () => {
    layerCounter++;
    const newLayer = { id: layerCounter, name: `Layer ${layerCounter}`, visible: true };
    layers._set([...layers.value, newLayer]);
    engine.emit(LayerSelected, layerCounter);
});
engine.on(LayerToggled, (id) => {
    layers._set(layers.value.map((l) => (l.id === id ? { ...l, visible: !l.visible } : l)));
});
// ---------------------------------------------------------------------------
// Canvas rendering via frame loop
// ---------------------------------------------------------------------------
export let canvasRef = null;
export function setCanvasRef(canvas) {
    canvasRef = canvas;
}
