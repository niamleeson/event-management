...
const SelectItem = engine.event('SelectItem');
const ItemSelected = engine.event('ItemSelected');
...
const autoRotateTween = engine.tween({
...
const baseAngle = engine.signal(DragMove, 0, (prev, dx) => prev + dx * 0.3);
// Track auto-rotate state
const autoRotating = engine.signal(AutoRotateToggle, true, (prev) => !prev);
...
// Selected item
const selectedItem = engine.signal(SelectItem, -1, (_prev, idx) => idx);
engine.pipe(SelectItem, ItemSelected, (idx) => idx);
// Selected item forward Z spring
const selectedZTarget = engine.signal(SelectItem, 0, (_prev, idx) => idx >= 0 ? 80 : 0);
const selectedZSpring = engine.spring(selectedZTarget, { stiffness: 170, damping: 20 });
...
export default function App() {
    ...
    return (_jsxs("div", { style: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 40, userSelect: 'none' }, onPointerDown: onPointerDown, onPointerMove: onPointerMove, onPointerUp: onPointerUp, children: [_jsx("h1", { style: { color: '#fff', fontSize: 28, fontWeight: 300, letterSpacing: 2 }, children: "3D Carousel" }), _jsx("div", { style: { perspective: 1200, width: 600, height: 300, position: 'relative' }, children: _jsx("div", { style: {
                        width: '100%',
                        height: '100%',
                        position: 'relative',
                        ...
                        return (_jsxs("div", { onClick: (e) => {
                            ...
                            }, children: [_jsx("span", { style: { fontSize: 40 }, children: item.icon }), _jsx("span", { style: { color: '#fff', fontSize: 16, fontWeight: 600, letterSpacing: 1 }, children: item.title })] }, i));
                    }) }) }), _jsxs("div", { style: { display: 'flex', gap: 16, alignItems: 'center' }, children: [_jsx("button", { onClick: (e) => {
                            e.stopPropagation();
                            ...
                            fontSize: 13,
                            letterSpacing: 1,
                        }, children: isAutoRotating ? '\u23F8 Pause' : '\u25B6 Resume' }), _jsxs("span", { style: { color: 'rgba(255,255,255,0.4)', fontSize: 13 }, children: ["Drag to control ", '\u00B7', " Click an item to select"] })] })] }));
}
