...
function createCardAnimations(eng) {
    const CardClicked = eng.event('CardClicked');
    const CardFlipped = eng.event('CardFlipped');
    const CardFlipDone = eng.event('CardFlipDone');
    const HoverIn = eng.event('HoverIn');
    const HoverOut = eng.event('HoverOut');
    const flipStarts = [];
    const unflipStarts = [];
    const flipTweens = [];
    const unflipTweens = [];
    ...
    for (let i = 0; i < 8; i++) {
        ...
        const ht = eng.signal(HoverIn, 1.0, (prev, idx) => idx === i ? 1.05 : prev);
...
function Card({ index }) {
    ...
    return (_jsx("div", { style: {
            ...
            height: 340,
            cursor: 'pointer',
        }, onClick: () => emit(anims.CardClicked, index), onMouseEnter: () => emit(anims.HoverIn, index), onMouseLeave: () => emit(anims.HoverOut, index), children: _jsxs("div", { style: {
                width: '100%',
                height: '100%',
                position: 'relative',
                ...
                borderRadius: 16,
                        ...
                        padding: 24,
                                ...
                                fontSize: 48,
                                marginBottom: 20,
                            }, children: card.title[0] }), _jsx("h3", { style: { color: '#fff', fontSize: 22, fontWeight: 700, letterSpacing: 1 }, children: card.title })] }), _jsxs("div", { style: {
                        position: 'absolute',
                        inset: 0,
                        backfaceVisibility: 'hidden',
                        borderRadius: 16,
                        ...
                        padding: 24,
                        gap: 16,
                    }, children: [_jsx("h3", { style: { color: card.color, fontSize: 20, fontWeight: 700 }, children: card.title }), _jsx("p", { style: { color: '#ccc', fontSize: 14, textAlign: 'center', lineHeight: 1.6, maxWidth: 200 }, children: card.desc }), _jsxs("div", { style: { display: 'flex', gap: 24, marginTop: 8 }, children: [_jsxs("div", { style: { textAlign: 'center' }, children: [_jsx("div", { style: { color: card.color, fontSize: 24, fontWeight: 700 }, children: card.views.toLocaleString() }), _jsx("div", { style: { color: '#888', fontSize: 11, textTransform: 'uppercase', letterSpacing: 1 }, children: "Views" })] }), _jsxs("div", { style: { textAlign: 'center' }, children: [_jsx("div", { style: { color: card.color, fontSize: 24, fontWeight: 700 }, children: card.likes.toLocaleString() }), _jsx("div", { style: { color: '#888', fontSize: 11, textTransform: 'uppercase', letterSpacing: 1 }, children: "Likes" })] })] })] })] }) }));
}
/* ------------------------------------------------------------------ */
/*  App                                                               */
/* ------------------------------------------------------------------ */
export default function App() {
    return (_jsxs("div", { children: [_jsx("h1", { style: { color: '#fff', textAlign: 'center', marginBottom: 32, fontSize: 28, fontWeight: 300, letterSpacing: 2 }, children: "3D Card Flip Gallery" }), _jsx("div", { style: {
                    display: 'grid',
                    gridTemplateColumns: 'repeat(4, 260px)',
                    gap: 24,
                }, children: CARDS.map((_, i) => (_jsx(Card, { index: i }, i))) })] }));
}
