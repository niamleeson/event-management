...
export default function App() {
  ...
  return (
    <div style={{ width: '100%', height: '100%', position: 'relative' }}>
      <ParticleCanvas />
      <div style={{ position: 'absolute', top: 24, left: 0, right: 0, display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 24, pointerEvents: 'none' }}>
        <h1 style={{ color: '#fff', fontSize: 22, fontWeight: 300, letterSpacing: 2 }}>Particle Explosion</h1>
        <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: 14 }}>Particles: {count}</span>
        <button onClick={() => emit(ClearAll, undefined)} style={{ pointerEvents: 'all', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', color: '#fff', padding: '8px 20px', borderRadius: 8, cursor: 'pointer', fontSize: 13, letterSpacing: 1 }}>Clear All</button>
      </div>
      <div style={{ position: 'absolute', bottom: 24, left: 0, right: 0, textAlign: 'center', color: 'rgba(255,255,255,0.3)', fontSize: 13, pointerEvents: 'none' }}>Click anywhere to spawn explosions</div>
    </div>
  )
}
