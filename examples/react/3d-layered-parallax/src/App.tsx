...
const LayerAnimDone = engine.event<number>('LayerAnimDone')
...
const mouseXTarget: Signal<number> = engine.signal(MouseMove, 0, (_prev, { x }) => (x - 0.5) * 15)
const mouseYTarget: Signal<number> = engine.signal(MouseMove, 0, (_prev, { y }) => (y - 0.5) * 10)

const cameraRotYSpring: SpringValue = engine.spring(mouseXTarget, { stiffness: 50, damping: 15 })
const cameraRotXSpring: SpringValue = engine.spring(mouseYTarget, { stiffness: 50, damping: 15 })

/* ------------------------------------------------------------------ */
/*  Entrance animation: staggered layer fade-in                       */
/* ------------------------------------------------------------------ */

const layerOpacityTweens: TweenValue[] = layerEnterEvents.map((startEvt, i) =>
  engine.tween({
    start: startEvt,
...
const isNight: Signal<boolean> = engine.signal(DayNightToggle, false, (prev) => !prev)

const dayNightTween: TweenValue = engine.tween({
...
const cloudFloat: TweenValue = engine.tween({
...
interface LayerDef {
  ...
  dayBg: string
  nightBg: string
  content: 'sky' | 'far-mountains' | 'near-mountains' | 'trees' | 'clouds'
}
  ...
  return (
    <div style={{ position: 'absolute', bottom: 0, left: '-10%', right: '-10%', height: '60%' }}>
      <svg viewBox="0 0 1200 400" preserveAspectRatio="none" style={{ width: '100%', height: '100%' }}>
        <path d={`M0,400 L0,280 Q150,120 300,200 Q450,80 600,180 Q750,60 900,160 Q1050,100 1200,220 L1200,400 Z`}
          fill={`rgb(${r},${g},${b})`} />
      </svg>
    </div>
  )
...
function NearMountains({ nightBlend }: { nightBlend: number }) {
  ...
  return (
    <div style={{ position: 'absolute', bottom: 0, left: '-10%', right: '-10%', height: '50%' }}>
      <svg viewBox="0 0 1200 400" preserveAspectRatio="none" style={{ width: '100%', height: '100%' }}>
        <path d={`M0,400 L0,300 Q100,180 250,250 Q400,120 550,220 Q700,140 850,200 Q1000,150 1200,260 L1200,400 Z`}
          fill={`rgb(${r},${g},${b})`} />
      </svg>
    </div>
  )
  ...
  return (
    <div style={{ position: 'absolute', bottom: 0, left: '-10%', right: '-10%', height: '35%' }}>
      <svg viewBox="0 0 1200 300" preserveAspectRatio="none" style={{ width: '100%', height: '100%' }}>
        ...
        <path d={`M0,300 L0,200 L30,120 L60,200 L80,100 L110,200 L140,140 L170,200 L200,80 L230,200 L260,150 L290,200 L320,110 L350,200 L380,130 L410,200 L440,90 L470,200 L500,160 L530,200 L560,100 L590,200 L620,140 L650,200 L680,70 L710,200 L740,150 L770,200 L800,120 L830,200 L860,90 L890,200 L920,160 L950,200 L980,110 L1010,200 L1040,130 L1070,200 L1100,80 L1130,200 L1160,140 L1200,200 L1200,300 Z`}
          fill={`rgb(${r},${g},${b})`} />
        {/* Ground */}
        <rect x="0" y="200" width="1200" height="100" fill={`rgb(${Math.round(34 + (10 - 34) * nightBlend)},${Math.round(139 + (20 - 139) * nightBlend)},${Math.round(34 + (10 - 34) * nightBlend)})`} />
      </svg>
    </div>
  )
}
...
export default function App() {
  ...
  const contentRenderers: Record<string, (nb: number, cf: number) => React.ReactNode> = {
    sky: (nb) => <SkyLayer nightBlend={nb} />,
    'far-mountains': (nb) => <FarMountains nightBlend={nb} />,
    'near-mountains': (nb) => <NearMountains nightBlend={nb} />,
    clouds: (nb, cf) => <Clouds nightBlend={nb} floatOffset={cf} />,
    trees: (nb) => <Trees nightBlend={nb} />,
  }
    ...
    >
      ...
      >
        {LAYERS.map((layer, i) => (
          ...
          >
            {contentRenderers[layer.content](actualNight, cloudFloatVal)}
          </div>
        ))}
      </div>
      ...
      <div style={{
        ...
        <h1 style={{ color: '#fff', fontSize: 22, fontWeight: 300, letterSpacing: 2, textShadow: '0 2px 8px rgba(0,0,0,0.5)' }}>
          3D Layered Parallax
        </h1>
        <button
          onClick={() => emit(DayNightToggle, undefined)}
          style={{
