  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(180deg, rgb(${c1.join(',')}) 0%, rgb(${c2.join(',')}) 100%)` }}>
      {nightBlend > 0.3 && Array.from({ length: 40 }, (_, i) => (
        <div key={i} style={{ position: 'absolute', left: `${(i * 37 + 13) % 100}%`, top: `${(i * 23 + 7) % 60}%`, width: 2, height: 2, borderRadius: '50%', background: '#fff', opacity: Math.max(0, (nightBlend - 0.3) / 0.7) * (0.3 + (i % 3) * 0.3) }} />
      ))}
    </div>
  )
}
...
function Clouds({ nightBlend, floatOffset }: { nightBlend: number; floatOffset: number }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      {clouds.map((c, i) => (
        <div key={i} style={{ position: 'absolute', left: c.left, top: c.top, width: c.w, height: c.h, borderRadius: '50%', background: `radial-gradient(ellipse, rgba(255,255,255,${opacity * 0.8}) 0%, rgba(255,255,255,${opacity * 0.2}) 70%, transparent 100%)`, transform: `translateY(${floatOffset * (i % 2 === 0 ? 1 : -0.7)}px)`, filter: `blur(${2 + i}px)` }} />
      ))}
    </div>
  )
}
...
export default function App() {
  ...
  return (
    <div onMouseMove={onMouseMove} style={{ width: '100%', height: '100%', perspective: 1000, overflow: 'hidden', cursor: 'crosshair' }}>
      <div style={{ width: '100%', height: '100%', transformStyle: 'preserve-3d', transform: `rotateX(${s.camRotX}deg) rotateY(${s.camRotY}deg)` }}>
        {LAYER_Z.map((z, i) => (
          <div key={i} style={{ position: 'absolute', inset: '-20%', transform: `translateZ(${z}px)`, opacity: s.layerOpacities[i] }}>{layers[i]}</div>
        ))}
      </div>
      <div style={{ position: 'absolute', top: 24, left: 0, right: 0, display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 24, zIndex: 10 }}>
        <h1 style={{ color: '#fff', fontSize: 22, fontWeight: 300, letterSpacing: 2, textShadow: '0 2px 8px rgba(0,0,0,0.5)' }}>3D Layered Parallax</h1>
        <button onClick={() => emit(DayNightToggle, undefined)} style={{ background: 'rgba(255,255,255,0.15)', border: '1px solid rgba(255,255,255,0.3)', color: '#fff', padding: '8px 20px', borderRadius: 8, cursor: 'pointer', fontSize: 13, letterSpacing: 1, backdropFilter: 'blur(4px)' }}>
          {s.isNight ? '\u2600 Day' : '\u263E Night'}
        </button>
      </div>
      <div style={{ position: 'absolute', bottom: 24, left: 0, right: 0, textAlign: 'center', color: 'rgba(255,255,255,0.4)', fontSize: 13, zIndex: 10, textShadow: '0 1px 4px rgba(0,0,0,0.5)' }}>Move mouse to tilt scene</div>
    </div>
  )
}
