...
// Event declarations
// ---------------------------------------------------------------------------
export const MetricReceived = engine.event('MetricReceived');
export const ThresholdBreached = engine.event('ThresholdBreached');
export const AlertCreated = engine.event('AlertCreated');
export const AlertDismissed = engine.event('AlertDismissed');
export const ChartDataUpdated = engine.event('ChartDataUpdated');
export const FeedToggled = engine.event('FeedToggled');
// ---------------------------------------------------------------------------
// Pipes & Rules
// ---------------------------------------------------------------------------
// MetricReceived -> ChartDataUpdated (always fires for every metric)
engine.pipe(MetricReceived, ChartDataUpdated, (metric) => ({
    name: metric.name,
...
let breachCounts = {};
engine.on(ThresholdBreached, (metric) => {
...
export const currentMetrics = engine.signal(MetricReceived, {}, (prev, metric) => ({ ...prev, [metric.name]: metric }));
// Alert list
export const alerts = engine.signal(AlertCreated, [], (prev, alert) => [alert, ...prev].slice(0, 20));
engine.signalUpdate(alerts, AlertDismissed, (prev, id) => prev.filter((a) => a.id !== id));
// Chart data (rolling window per metric)
export const chartData = engine.signal(ChartDataUpdated, Object.fromEntries(METRICS.map((m) => [m.name, []])), (prev, update) => {
    const existing = prev[update.name] ?? [];
    const next = [...existing, update.point].slice(-ROLLING_WINDOW);
    return { ...prev, [update.name]: next };
});
// Feed running state
export const feedRunning = engine.signal(FeedToggled, true, (_prev, running) => running);
// ---------------------------------------------------------------------------
