...
@Component({
  ...
  template: `
    <div class="page">
      ...
      <p class="subtitle">Auto-rotates with tween. Drag to override. Click to select (springs forward).</p>
      ...
      @if (selected() >= 0) {
        <div class="selected-info">
          Selected: <strong>{{ items[selected()].title }}</strong>
        </div>
      }
    </div>
  `,
...
export class ThreeDCarouselComponent implements OnInit, OnDestroy {
  ...
  rot: WritableSignal<number>
  selected: WritableSignal<number>
  depth: WritableSignal<number>

  private dragging = false
  private dragStartX = 0

  constructor(private pulse: PulseService) {
    this.rot = pulse.spring(springRotation)
    this.selected = pulse.signal(selectedItem)
    this.depth = pulse.spring(springDepth)
  }
  ...
  onMouseDown(e: MouseEvent): void {
    ...
    this.pulse.emit(DragStart, e.clientX)
  }
  ...
  onMouseMove(e: MouseEvent): void {
    ...
    this.pulse.emit(DragMove, delta)
  }
  ...
  selectCard(index: number, e: MouseEvent): void {
    e.stopPropagation()
    this.pulse.emit(SelectItem, index)
  }
}
