...
@Component({
        ...
        <div class="cube" [style.transform]="'rotateX(' + rotX() + 'deg) rotateY(' + rotY() + 'deg)'">
          @for (face of faces; track $index; let i = $index) {
            <div
              ...
              [style.box-shadow]="selected() === i ? '0 0 ' + (20 + glow() * 20) + 'px ' + face.color : 'none'"
            >
  ...
  styles: [`
    ...
    .face-0 { transform: translateZ(125px); }
    .face-1 { transform: rotateY(90deg) translateZ(125px); }
    .face-2 { transform: rotateY(180deg) translateZ(125px); }
    .face-3 { transform: rotateY(-90deg) translateZ(125px); }
    .face-4 { transform: rotateX(-90deg) translateZ(125px); }
...
export class ThreeDCubeMenuComponent implements OnInit, OnDestroy {
  faces = FACES
  rotX: WritableSignal<number>
  rotY: WritableSignal<number>
  selected: WritableSignal<number>
  glow: WritableSignal<number>
  ...
  constructor(private pulse: PulseService) {
    this.rotX = pulse.spring(springRotX)
    this.rotY = pulse.spring(springRotY)
    this.selected = pulse.signal(selectedFace)
    this.glow = pulse.spring(glowSpring)
  }

  ngOnInit(): void {
    (window as any).__pulseEngine = engine
    // Initialize to face 0
  ...
  onMouseDown(e: MouseEvent): void {
    ...
    this.dragAccX = targetRotX.value
    this.dragAccY = targetRotY.value
  ...
  onMouseMove(e: MouseEvent): void {
    ...
    this.dragAccX -= dy * 0.5
    this.dragAccY += dx * 0.5
    this.lastX = e.clientX
    this.lastY = e.clientY
  ...
  goToFace(index: number): void {
    this.pulse.emit(SnapToFace, index)
    this.pulse.emit(SelectFace, index)
  }
}
