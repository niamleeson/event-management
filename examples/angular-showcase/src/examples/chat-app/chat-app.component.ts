import { Component, type WritableSignal, OnInit, OnDestroy, signal as ngSignal, AfterViewInit, ViewChild, ElementRef } from '@angular/core'
    ...
    <div class="page">
      <div class="chat-container">
        <div class="chat-header">
          <h2>Pulse Chat</h2>
          <span class="unread-badge" [class.visible]="unread() > 0">{{ unread() }}</span>
        </div>
        <div class="messages" #messageList>
          @for (msg of msgs(); track msg.id) {
            <div
              ...
              [class.bot]="msg.sender !== 'You'"
              (mouseenter)="markRead(msg)"
            >
              ...
              <div class="msg-meta">
                ...
                @if (msg.sender === 'You') {
                  <span class="msg-status">{{ msg.read ? 'Read' : 'Sent' }}</span>
                }
                @if (msg.sender !== 'You') {
                  <span class="msg-status" [class.unread]="!msg.read">
                    {{ msg.read ? 'Read' : 'New' }}
                  </span>
                }
              </div>
...
export class ChatAppComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  msgs: WritableSignal<Message[]>
  typing: WritableSignal<Set<string>>
  ...
  constructor(private pulse: PulseService) {
    this.msgs = pulse.signal(messages)
    this.typing = pulse.signal(typingUsers)
    this.unread = pulse.signal(unreadCount)
  }
  ...
  send(): void {
    ...
    if (!text) return
    this.pulse.emit(SendMessage, { text, sender: 'You' })
    this.pulse.emit(MessageEnter, '')
    this.inputText = ''
  }

  markRead(msg: Message): void {
    if (!msg.read && msg.sender !== 'You') {
      this.pulse.emit(MarkRead, msg.id)
    }
  }
