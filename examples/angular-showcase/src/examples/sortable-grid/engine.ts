...
engine.on(Reorder, ({ fromIndex, toIndex }) => { const r = [...items]; const [m] = r.splice(fromIndex, 1); r.splice(toIndex, 0, m); items = r; engine.emit(ItemsChanged, items) })
engine.on(Shuffle, () => { const s = [...items]; for (let i = s.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [s[i], s[j]] = [s[j], s[i]] }; items = s; engine.emit(ItemsChanged, items) })
engine.on(AddItem, () => { const id = String.fromCharCode(97 + nextId); nextId++; items = [...items, { id, label: 'Item ' + id.toUpperCase(), color: NEW_COLORS[items.length % NEW_COLORS.length] }]; engine.emit(ItemsChanged, items) })
engine.on(RemoveItem, (id) => { items = items.filter((i) => i.id !== id); engine.emit(ItemsChanged, items) })
