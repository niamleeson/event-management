import {
  ...
  type Alert,
  type ChartDataPoint,
} from '../engines/realtime-dashboard'
...
export function mount(container: HTMLElement): () => void {
  ...
  toggleBtn.addEventListener('click', () => engine.emit(FeedToggled, !feedRunning.value))
  ...
  const metricCards: Record<string, { card: HTMLElement; valueEl: HTMLElement; thresholdEl: HTMLElement }> = {}
  ...
  for (const config of METRICS) {
    ...
    nameEl.style.cssText = 'font-size: 13px; font-weight: 600; color: #64748b; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px;'
    ...
    thresholdEl.style.cssText = 'font-size: 12px; color: #475569; margin-top: 4px;'
    thresholdEl.textContent = `Threshold: ${config.threshold}${config.unit}`
    ...
    metricCards[config.name] = { card, valueEl, thresholdEl }
  }
  ...
  for (const config of METRICS) {
    ...
    chartTitle.style.cssText = 'font-size: 14px; font-weight: 600; color: #94a3b8; margin-bottom: 16px;'
    chartTitle.textContent = config.name
  ...
  function updateCharts() {
    ...
    for (const config of METRICS) {
      ...
      const { barsArea } = chartElements[config.name]
      const color = CHART_COLORS[config.name] ?? '#4361ee'

      if (points.length === 0) {
        barsArea.innerHTML = ''
        const empty = document.createElement('div')
        empty.style.cssText = 'width: 100%; display: flex; justify-content: center; align-items: center; color: #334155; font-size: 13px;'
      ...
      // Rebuild bars (data changes every second, not every frame)
      barsArea.innerHTML = ''
      for (let i = 0; i < points.length; i++) {
        const point = points[i]
        const height = (point.value / maxVal) * 100
        const breached = point.value > config.threshold
      ...
      // Remove old threshold line if exists
      const oldLine = chartElements[config.name].container.querySelector('.threshold-line')
      if (oldLine) oldLine.remove()
      thresholdLine.className = 'threshold-line'
      chartElements[config.name].container.appendChild(thresholdLine)
    }
  }

  function updateAlerts() {
    const alertList = alerts.value as Alert[]
    ...
    if (alertList.length === 0) {
      const empty = document.createElement('div')
      empty.style.cssText = 'text-align: center; padding: 32px; color: #334155; font-size: 14px;'
      empty.textContent = 'No active alerts. Alerts trigger after 3 consecutive threshold breaches.'
    ...
    for (const alert of alertList) {
      ...
      msgEl.style.cssText = 'font-size: 14px; color: #fca5a5; flex: 1;'
      msgEl.textContent = alert.message

      const timeEl = document.createElement('span')
      timeEl.style.cssText = 'font-size: 12px; color: #7f1d1d; margin-left: 16px; flex-shrink: 0;'
      timeEl.textContent = new Date(alert.timestamp).toLocaleTimeString()

      const dismissBtn = document.createElement('button')
      dismissBtn.style.cssText = 'font-size: 18px; color: #7f1d1d; background: none; border: none; cursor: pointer; margin-left: 12px; padding: 0 4px; transition: color 0.2s;'
      dismissBtn.textContent = 'x'
      dismissBtn.addEventListener('click', () => engine.emit(AlertDismissed, alert.id))
      dismissBtn.addEventListener('mouseenter', () => { dismissBtn.style.color = '#fca5a5' })
      dismissBtn.addEventListener('mouseleave', () => { dismissBtn.style.color = '#7f1d1d' })

      alertCard.appendChild(msgEl)
      alertCard.appendChild(timeEl)
  ...
  // Subscribe
  unsubs.push(feedRunning.subscribe(() => updateHeader()))
  unsubs.push(currentMetrics.subscribe(() => updateMetrics()))
  unsubs.push(chartData.subscribe(() => updateCharts()))
  unsubs.push(alerts.subscribe(() => updateAlerts()))
