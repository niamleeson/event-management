import type { Signal, SpringValue, EventType } from './types.js'

/**
 * Create a SpringValue — a physics-driven animated value.
 */
export function createSpring(config: {
  target: Signal<number> | { value: number; subscribe: (cb: (v: number) => void) => () => void }
  stiffness: number
  damping: number
 ...
 */
export function advanceSpring(spring: SpringValue, dt: number): boolean {
  ...
  // Check if settled
  if (
    Math.abs(spring.velocity) < spring._restThreshold &&
